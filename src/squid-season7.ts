...
query GetYokiPerSeasons($limit: Int!, $offset: Int!) {
  yokiPerSeasons(
    where: {season_eq: 7, hasAll_eq: true}
    limit: $limit
    offset: $offset
  ) {
    address
...
async function main() {
  try {
    ...
    // Write to file
    const outputPath = path.join(dataDir, '_season7users.csv');
    fs.writeFileSync(outputPath, csvContent);

    console.log(`Success! Exported ${addresses.length} addresses to _season7users.csv`);
  } catch (error: any) {
