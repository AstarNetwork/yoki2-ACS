...
query GetYokiPerSeasons($limit: Int!, $offset: Int!) {
  yokiPerSeasons(
    where: {season_eq: 1, hasAll_eq: true}
    limit: $limit
    offset: $offset
    orderBy: address_ASC
  ) {
...
async function fetchAllUsers(): Promise<string[]> {
  ...
  console.log('Fetching Season 1 OG qualifying users...');
  ...
  while (hasMore) {
    try {
      ...
      const addresses = users.map((user: Season8User) => user.address);
...
async function main() {
  try {
    ...
    // Write to file
    const outputPath = path.join(dataDir, '_season1users.csv');
    fs.writeFileSync(outputPath, csvContent);

    console.log(`Success! Exported ${addresses.length} addresses to _season1users.csv`);
  } catch (error: any) {
